...
Cota's use of a single frequency simplifies the process because it means they only have to adjust the phase of the oscillator driving each antenna in the array. In addition, it allows them to work peacefully in a small pocket the 2.4~GHz ISM band, without interfering with the majority of the spectrum that is utilized by Wi-Fi devices. In addition, Zeine claims that lower frequencies have longer wavelengths that preclude the formation of ``small'' pockets of energy, while higher frequencies (such as 24~GHz) have much more stringent SAR limits set in place by the FCC.
